/**
 * Normalize Vietnamese text to English without diacritics
 * Converts: "ạ" -> "a", "đ" -> "d", etc.
 */
export function normalizeToEnglish(text: string): string {
  ...
  for (const [viet, eng] of Object.entries(vietnameseMap)) {
    normalized = normalized.split(viet).join(eng);
  }

  // Remove any remaining diacritics using Unicode normalization
  normalized = normalized.normalize('NFD').replace(/[\u0300-\u036f]/g, '');

  // Keep only alphanumeric, space, and basic punctuation
