      ...
      <div className="grid grid-cols-2 gap-6 mb-8">
        ...
        <div className="border-2 border-gray-400 rounded-lg p-4 bg-gray-50">
          ...
          {Array.from({ length: 12 }, (_, rowIndex) => (
            <div key={`left-${rowIndex}`} className="grid grid-cols-2 gap-2 mb-2">
              {['P1', 'P2'].map(position => {
                const entry = getEntry(rowIndex, 'left', position);
                if (!entry) return null;
                
                return (
                  <div key={entry.id} className="border-2 border-gray-400 rounded p-2 bg-white shadow-sm">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      />
                      <span className="text-sm font-bold text-gray-700">#{rowIndex + 1}</span>
                    </div>
                    <input
                      type="text"
                      value={entry.text}
        ...
        <div className="border-2 border-gray-400 rounded-lg p-4 bg-gray-50">
          ...
          {Array.from({ length: 12 }, (_, rowIndex) => (
            <div key={`right-${rowIndex}`} className="grid grid-cols-2 gap-2 mb-2">
              {['P1', 'P2'].map(position => {
                const entry = getEntry(rowIndex, 'right', position);
                if (!entry) return null;
